...
export const createCva = (config: Cva, mergeCss: MacroContext['mergeCss']) => {
...
function getCompoundVariantCss(
  ...
  mergeCss: MacroContext['mergeCss'],
) {
  let result = {}
...
export const transformCva = (
  ...
  return `(function () {
    const base = ${JSON.stringify(resolve(base))}
    const variantStyles = ${JSON.stringify(
      Object.fromEntries(
        Object.entries(variants).map(([variantKey, variantMap]) => [
          variantKey,
          Object.fromEntries(
            Object.entries(variantMap).map(([valueKey, variantStyle]) => [valueKey, resolve(variantStyle)]),
          ),
        ]),
      ),
      null,
